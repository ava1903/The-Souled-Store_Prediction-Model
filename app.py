...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    with sc1: target_b  = st.slider("Target Avg. Fandom Breadth",1.0,3.0,1.8,0.1)
    with sc2: conv_rate = st.slider("Cross-Fandom Conversion Rate (%)",2,20,8,1)
    with sc3: sim_aov   = st.slider("Average Order Value (₹)",400,1200,650,50)
    extra = max(0,(target_b-cur_breadth)*len(filt))
    uplift_rev = extra*(conv_rate/100)*sim_aov/1e5
    r1,r2,r3 = st.columns(3)
    with r1: st.metric("Breadth Gap to Close", f"{max(0,target_b-cur_breadth):.2f} IPs/customer")
    with r2: st.metric("Estimated Extra Purchases",f"{int(extra):,}")
    ...
    with r2:
        ...
        sdf    = pd.DataFrame({"fandom":FANDOMS,"score":scores})
        sdf    = sdf[sdf["fandom"]!=sf].sort_values("score",ascending=False)
